...
class XunFeiRTASR {
  ...
  Future<void> startRecording(String code, int punc, String path) async {
    ...
    if (path.isNotEmpty) {
      _audioFileWriter = AudioFileWriter();
      await _audioFileWriter!.startWriting(path);
    }
    ...
    _audioStreamController!.stream.listen((audioData) async {
      ...
      try {
        if (_channel == null ||
            _channel!.innerWebSocket?.readyState != WebSocket.open) {
          print('${getCurrentTimeStr()} WebSocket 已关闭，跳过数据发送');
          return;
        }
        if (_audioFileWriter != null)
          await _audioFileWriter!.writeAudioData(audioData);
        if (audioData.length > chunkedSize) {
          final chunks = audioData.length ~/ chunkedSize;
          for (int i = 0; i < chunks; i++) {
            final chunk =
            audioData.sublist(i * chunkedSize, (i + 1) * chunkedSize);
            send(_channel!, chunk);
          }
          final remaining = audioData.length % chunkedSize;
          if (remaining > 0) {
            final chunk = audioData.sublist(chunks * chunkedSize);
            send(_channel!, chunk);
          }
        } else {
          send(_channel!, audioData);
        }
      } catch (e) {
  ...
  Future<void> stopWriteData() async {
    ...
    if (_audioFileWriter != null) await _audioFileWriter?.stopWriting();
    ...
    if (_channel != null &&
        _channel!.innerWebSocket?.readyState == WebSocket.open) {
      send(_channel!, utf8.encode('{"end": true}'));
  ...
  static void send(IOWebSocketChannel channel, List<int> bytes) {
    if (channel.innerWebSocket == null ||
        channel.innerWebSocket!.readyState != WebSocket.open) {
      print('${getCurrentTimeStr()} WebSocket 连接已关闭，无法发送数据');
      return;
    }
