...
class _QuickStartPageState extends State<QuickStartPage> {
  ...
  Future<void> loginRoom() async{
    ...
    if (kIsWeb) {
      ...
      final token = (await generateZegoToken(ZegoConfig.instance.appID, ZegoConfig.instance.userID, ZegoConfig.instance.secret, 3600, ""))!;
  ...
  void startObserver() {
    ...
    this.isOpenObserver = !this.isOpenObserver;
  }
  // MARK: - Exit
  ...
  void setZegoEventCallback() {
    ...
    ((roomID, updateType, streamList, extendedData) {
      streamList.forEach((stream) {
        ...
        print(
            '🚩 🚪 fffflutter Room stream update, roomID: $roomID, updateType: $updateType streamID:$streamID');
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Row(children: [
        ...
        Container(
          width: MediaQuery.of(context).size.width  * (1/8),
          child: CupertinoButton.filled(
            child: Text('startAudioData',
              style: TextStyle(fontSize: 14.0),
            ),
            onPressed: startObserver,
            padding: EdgeInsets.all(10.0),
          ),
        )
      ]),
